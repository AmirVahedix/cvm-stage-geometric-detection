#!/usr/bin/env python3
"""
Batch Inference Script for CVM Stage Geometric Detection.

Workflow:
1. Loads test task/image IDs from test_image_ids.json.
2. Authenticates with Label Studio using credentials from .env.
3. Retrieves project export (Project ID 1) with all human-annotated landmarks.
4. Concurrently downloads test images to data/test/images/{id}.jpg.
5. Extracts the 13 cervical vertebrae keypoint coordinates.
6. Executes the geometric CVM calculator (src.cvm_calculator) to classify CVM stage (CS1-CS6).
7. Stores the determined integer stage (1-6) in data/test/labels/{id}.txt.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Batch download test images from Label Studio and determine CVM stages using geometric rules."
    )
    parser.add_argument(
        "--test-ids",
        type=str,
        default="test_image_ids.json",
        help="Path to JSON file containing list of test image/task IDs.",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        default="data/test",
        help="Target output directory for images and labels (default: data/test).",
    )
    parser.add_argument(
        "--project-id",
        type=int,
...
def extract_landmarks(
    ...
    for res in results:
        ...
        if lbl_name in EXPECTED_LANDMARKS:
            ...
            # Store temporarily with norm coords if dims not known yet
...
def main():
    ...
    # 2. Load test image IDs
    test_ids_path = Path(args.test_ids)
    if not test_ids_path.exists():
        fallback_path = Path("../swin-gcn-network/dataset/test_image_ids.json")
        if fallback_path.exists():
            test_ids_path = fallback_path
        else:
            print(f"❌ Error: Test IDs file not found: {args.test_ids}", file=sys.stderr)
            sys.exit(1)

    with open(test_ids_path, "r", encoding="utf-8") as f:
        test_ids_raw = json.load(f)
    test_ids = [int(x) if str(x).isdigit() else str(x) for x in test_ids_raw]
    test_ids_set = set(test_ids)
    print(f"📋 Loaded {len(test_ids)} test task IDs from: {test_ids_path}")
    ...
    # 4. Initialize session and fetch project export
    ...
    matched_tasks: List[Tuple[Any, Dict[str, Any]]] = []
    missing_ids: List[Any] = []

    for tid in test_ids:
        if tid in tasks_by_id:
            matched_tasks.append((tid, tasks_by_id[tid]))
        else:
            missing_ids.append(tid)

    if missing_ids:
        print(f"⚠️ Warning: {len(missing_ids)} test IDs were not found in project export: {missing_ids}")
    print(f"🎯 Matched {len(matched_tasks)}/{len(test_ids)} tasks in project export.")

    # 5. Concurrently download images
    print(f"\n⬇️ Downloading {len(matched_tasks)} test images to {images_dir}...")
    task_image_paths: Dict[Any, Path] = {}

    def download_worker(tid_task):
        tid, task = tid_task
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as executor:
        futures = {executor.submit(download_worker, item): item[0] for item in matched_tasks}
    ...
    # 6. Process landmarks and calculate CVM stages
    print(f"\n📐 Computing CVM stages using geometric formulas for {len(matched_tasks)} images...")
    stage_counts = Counter()
    successful = 0
    errors = []

    for tid, task in tqdm(matched_tasks, desc="Evaluating CVM Stages", unit="sample"):
        ...
        try:
            ...
            # Write single integer label file: data/test/labels/{tid}.txt
    ...
    print(f"Images Downloaded:      {len(task_image_paths)} (stored in {images_dir})")
    ...
    print("=" * 50)
    print("🎉 Done! All test images and CVM label files successfully generated.")
